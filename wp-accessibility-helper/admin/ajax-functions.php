...
function wah_scan_homepage() {
	...
	if ( $body && $post_id && $response_code == 200 ) {
		...
		if ( $scanner_array['images'] ) {
			...
			<div class="form_row">
				<h3 class="wah_scanner_table_trigger">
					<span></span><?php esc_html_e( 'Images Report Table', 'wp-accessibility-helper' ); ?>
				</h3>
				<table class="widefat fixed wah_scanner_table" cellspacing="5">
					...
					<tbody>
						<?php
						foreach ( $scanner_array['images'] as $key => $image_html ) :
						...
						<td>
							<a target="_blank" href="<?php echo add_query_arg( 'wahi', base64_encode( $src ), get_permalink( $post_id ) ); ?>">
								<?php _e( 'Preview', 'wp-accessibility-helper' ); ?>
							</a>
						</td>
						</tr>
				...
				<tbody>
					...
					foreach ( $scanner_array['links'] as $key => $link_html ) :
						...
						<tr>
							...
							<td>
								<a href="<?php echo add_query_arg( 'wahl', base64_encode( $href ), get_permalink( $post_id ) ); ?>" target="_blank">
