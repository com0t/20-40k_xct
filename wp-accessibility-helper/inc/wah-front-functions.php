...
function run_front_dom_scanner() {
	wp_register_style( 'wah_analyzer-styles', plugins_url() . '/wp-accessibility-helper/admin/wah-analyzer/style.css', array(), WAH_VERSION );
	wp_enqueue_style( 'wah_analyzer-styles' );

	wp_localize_script( 'wah_analyzer-js', 'ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
	wp_register_script( 'wah_analyzer-js', plugins_url() . '/wp-accessibility-helper/admin/wah-analyzer/wah_analyzer.js', array( 'jquery' ), WAH_VERSION, true );
					...
					<div class="color_selector" aria-hidden="true">
						...
						<?php else : ?>
							<?php foreach ( $contrast_variations as $contrast ) : ?>
								<button type="button" class="convar wahout wahcolor" style="background:#<?php echo $contrast['bgcolor']; ?> !important;" data-bgcolor="#<?php echo $contrast['bgcolor']; ?>" data-color="#<?php echo $contrast['textcolor']; ?>" title="<?php echo esc_html( $contrast['title'] ); ?>">
